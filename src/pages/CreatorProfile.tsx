...
export default function CreatorProfile() {
  ...
  // For demo creators, use the static data
      ...
      <div className="px-4 py-6 safe-top">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="flex flex-col items-center mb-6">
          <div className="w-24 h-24 rounded-full bg-gradient-to-br from-primary to-amber-500 flex items-center justify-center mb-4">
            <span className="text-4xl font-bold text-primary-foreground">{creator.avatar}</span>
          </div>
          <h1 className="font-display text-2xl font-bold text-foreground mb-1">@{creator.username}</h1>
          <p className="text-muted-foreground text-sm text-center max-w-[250px]">{creator.bio}</p>
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}
          ...
          <div className="text-center">
            ...
            <p className="text-xs text-muted-foreground flex items-center gap-1"><Eye className="w-3 h-3" /> Views</p>
          </div>
        </motion.div>

        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}>
          <h2 className="font-display text-lg font-semibold text-foreground mb-4">Posts ({creatorPosts.length})</h2>
          {creatorPosts.length === 0 ? (
            ...
            <div className="grid grid-cols-3 gap-1">
              {creatorPosts.map((post) => (
                <div key={post.id} className="aspect-square bg-secondary rounded-lg overflow-hidden relative cursor-pointer" onClick={() => navigate("/home")}>
        ...
        <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.4 }}
          className="text-xs text-muted-foreground text-center mt-8">Beta profile</motion.p>
      </div>
    </MobileLayout>
  );
}
